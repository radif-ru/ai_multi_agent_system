...
- [ ] `flake8 app` — 0 нарушений.
- [ ] `pytest -q` — зелёный.
- [ ] `git diff --stat` показывает только удаления/мелкие правки, никаких рефакторингов.
- [ ] **Документация обновлена** — `n/a` (правки не меняют поведение).
...
- [ ] `flake8 app tests` — 0 нарушений.
- [ ] В `.flake8` нет «мёртвых» исключений.
- [ ] **Документация обновлена** — `n/a`.
- [ ] **Тесты добавлены / обновлены** — `n/a`.
- [ ] `git status` чист.
...
- [ ] Описание схемы и назначение каждого поля в `_docs/memory.md` соответствует коду.
- [ ] Зафиксирован вердикт: можно ли удалить `chat_id` без потери функциональности (или почему он остаётся).
- [ ] **Документация обновлена** — да, `_docs/memory.md`.
- [ ] **Тесты добавлены / обновлены** — `n/a` (только аудит).
- [ ] `git status` чист.
...
- [ ] В `_docs/memory.md` §2.6 актуальное описание схемы.
- [ ] Если найдено мёртвое поле — запись в `_docs/current-state.md` §2 с предложением удалить отдельной задачей.
...
- [ ] `pytest tests/services/test_memory.py -q` — зелёный, есть кейс с фильтром `scope_user_id`.
- [ ] **Документация обновлена** — `n/a`.
- [ ] **Тесты добавлены / обновлены** — да (или `n/a` с обоснованием).
- [ ] `git status` чист.
...
- [ ] Файл `app/services/dialog_journal.py` создан, schema идемпотентна (`CREATE IF NOT EXISTS`).
- [ ] Unit-тесты на `tmp_path` БД: append → pending → read → mark_archived.
- [ ] `_docs/memory.md` §4 — описание журнала, его места в архитектуре, инвариантов.
- [ ] `pytest -q` зелёный, `flake8` зелёный.
- [ ] `git status` чист.
...
|-----|-------------------------------------------------------|:---------:|:-----:|:------:|:----------:|
| 1.1 | Закрыть нарушения flake8 в `app/`                     | medium    | S     | Progress | —        |
| 1.2 | Закрыть нарушения flake8 в `tests/`                   | medium    | S     | ToDo   | —          |
| 1.3 | Пересмотреть `per-file-ignores` в `.flake8`           | low       | XS    | ToDo   | 1.1, 1.2   |
| 2.1 | Аудит `data/memory.db`                                | high      | S     | ToDo   | —          |
| 2.2 | Аудит `data/file_contexts.db`                         | medium    | XS    | ToDo   | —          |
| 2.3 | Сквозной тест RAG-поиска                              | medium    | S     | ToDo   | 2.1        |
| 3.1 | Схема `dialog_journal` и API                          | high      | M     | ToDo   | 2.1        |
