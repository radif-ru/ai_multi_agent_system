...
- [ ] `ConversationStore` больше не открывает `data/file_contexts.db` (импорт `sqlite3` остаётся только если нужен для других целей).
- [ ] `get_file_context` тянет данные из `dialog_journal`; reply на файл по `message_id` продолжает работать (покрыто тестом).
- [ ] `save_file_context` удалён (или превращён в no-op deprecated с предупреждением — на усмотрение исполнителя, предпочтительно удалить).
- [ ] `_docs/memory.md` §2.6 актуализирован: `_file_contexts` — только in-memory кеш, источник истины — `dialog_journal`.
...
| 3-bis.2 | `ConversationStore` — на `dialog_journal`         | high      | S     | Progress | 3-bis.1  |
