...
- [ ] `.ocr.txt`-файлы больше не создаются (проверено тестом OCR-тулов с временной директорией).
- [ ] Производительность не деградирует на типовых сценариях (OCR одного файла на ход — основной путь).
- [ ] `pytest -q`, `flake8` зелёные.
- [ ] `_docs/tools.md` обновлён (упоминания кеша убраны или помечены устаревшими).
- [ ] `git status` чист.
...
| 3-bis.4 | Отказ от `.ocr.txt` на диске                      | medium    | XS    | Progress | —        |
