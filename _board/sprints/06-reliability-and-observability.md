...
- [ ] Колонка `message_id` и индекс созданы; повторный старт не падает.
- [ ] `MessageReceived` несёт `message_id`; подписчик пишет его в журнал; в трёх file-хендлерах поле проброшено.
- [ ] Миграция отрабатывает один раз; при следующих стартах файл `.migrated-*` игнорируется.
- [ ] Unit-тесты на миграцию (пустой файл, одна запись, повторный старт) и на `append(message_id=...)`.
- [ ] `_docs/memory.md` §4.1 обновлён (новая колонка, индекс).
...
| 3-bis.1 | Расширить `dialog_journal` колонкой `message_id` + миграция | high | S | ToDo | 3.1, 3.3 |
