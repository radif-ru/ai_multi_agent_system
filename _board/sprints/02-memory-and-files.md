...
- [ ] `parse_agent_response` корректно парсит ответ вида ` ```json\n{...}\n``` `, ` ```\n{...}\n``` ` и вариант с обрамляющими пробелами.
- [ ] Бэк-компат: голый JSON-ответ парсится как раньше.
- [ ] Невалидный внутри fence JSON → по-прежнему `LLMBadResponse` с осмысленным сообщением.
- [ ] `_prompts/agent_system.md` явно запрещает markdown-код-блоки в ответе.
- [ ] `_docs/agent-loop.md` §2 обновлён.
- [ ] `_docs/current-state.md` §2.2 перенесён в §6 с SHA.
- [ ] Тесты в `tests/agents/test_protocol.py` покрывают оба fence-варианта и back-compat.
- [ ] `pytest -q` зелёный.
...
| 5.1 | Снятие markdown-fence в `parse_agent_response`         | high      | S     | Progress   | —                            |
