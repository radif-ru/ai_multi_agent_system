...
1. **Память не доходит до LLM.** `ConversationStore` и `SemanticMemory` реализованы и покрыты тестами в Спринте 01, но `Executor.run` каждый раз собирает `messages = [system, user_goal]` с нуля (`app/agents/executor.py:75-78`), а долгосрочная память подключена только через tool `memory_search`, которое модель должна вызвать сама. На практике это даёт эффект «каждое сообщение — новая сессия» (см. скриншот пользователя).
2. **Бот не принимает файлы.** Telegram-handler в `app/adapters/telegram/handlers/messages.py` ловит любое сообщение, но при `message.text is None` отвечает `«В MVP я понимаю только текст»` и не умеет работать с `Photo` / `Voice` / `Document`. Документация `_docs/roadmap.md` Этап 7 предусматривала отдельный спринт; пользователь явно попросил начать сейчас.
...
- [x] В переписке `Привет, я Радиф / Как меня зовут?` бот отвечает корректным именем, не теряет контекст между ходами.
- [x] После `/new` и нового вопроса по теме прошлой сессии бот **сам** подмешивает релевантные чанки из `data/memory.db` (через авто-подгрузку или явный вызов `memory_search`) — проверяется тестом и smoke-проверкой.
- [x] При `/new` в долгосрочную память уходит **вся** история сессии, а не усечённая `replace_with_summary` верхушка: имя/факты, упомянутые в начале длинной сессии, доступны через `memory_search` / авто-подгрузку после `/new` (регрессионный тест).
- [x] Бот принимает PDF/TXT/MD как `Document` и отвечает по содержимому.
- [x] Бот принимает голосовое (`Voice`) и распознаёт речь в текст, текст уходит в агентный цикл.
- [x] Бот принимает фото (`Photo`) и либо распознаёт через vision-модель, либо отвечает понятным сообщением «vision-модель не настроена», без падения.
- [x] Превышение `TELEGRAM_MAX_FILE_MB` → понятное сообщение пользователю, файл не скачивается.
- [x] `pytest -q` зелёный, новые компоненты покрыты unit-тестами.
- [x] `_docs/agent-loop.md`, `_docs/memory.md`, `_docs/architecture.md`, `_docs/commands.md`, `_docs/tools.md`, `_docs/requirements.md`, `README.md` соответствуют коду.
- [x] `_board/progress.txt` дополнен чек-листом приёмки Спринта 02.
- [x] Все задачи спринта — `Done`.
...
- [x] Tool реализован и зарегистрирован.
- [x] Тесты: PDF, TXT, неизвестное расширение → `ToolError`, превышение `max_chars` → усечение, попытка выхода за `tmp_files_dir` → `ToolError`.
- [x] `pytest -q` зелёный.
...
- [x] Handler реализован, регистрируется в `messages_router`.
- [x] Тест: получение `Document`, проверка вызова `core.handle_user_task` с обогащённым текстом.
- [x] Тест: превышение `TELEGRAM_MAX_FILE_MB` → понятное сообщение, executor не вызывается.
- [x] `pytest -q` зелёный.
...
- [x] `Transcriber` реализован; в тестах используется мок (без скачивания моделей).
- [x] Handler `Voice` реализован и зарегистрирован.
- [x] Тест: успешная транскрипция → `core.handle_user_task` вызывается с распознанным текстом.
- [x] Тест: `Transcriber` недоступен → понятное сообщение пользователю.
- [x] `pytest -q` зелёный.
...
- [x] `Vision.describe(path) -> str` реализован, использует `OllamaClient`.
- [x] Handler `Photo` реализован и зарегистрирован.
- [x] Тест: успешное описание (с моком LLM) → текст уходит в агентный цикл.
- [x] Тест: `VISION_MODEL` пуст → fallback-сообщение.
- [x] `pytest -q` зелёный.
...
- [x] Все перечисленные документы обновлены.
- [x] Чек-лист в `progress.txt` заполнен.
- [x] Тесты: n/a (документация).
...
`parse_agent_response` (`app/agents/protocol.py:42`) делает чистый `json.loads(text)` и падает → `LLMBadResponse` → пользователю уходит `LLM_BAD_RESPONSE_REPLY` («Модель ответила в неожиданном формате»).
...
| 3.2 | Tool `read_document` (PDF/TXT/MD)                    | high      | M     | Done   | 3.1                          |
| 3.3 | Handler `Document`                                   | high      | S     | Done   | 3.1, 3.2                     |
| 3.4 | Handler `Voice` (faster-whisper)                     | high      | L     | Done   | 3.1                          |
| 3.5 | Handler `Photo` (vision)                             | medium    | M     | Done   | 3.1                          |
| 3.6 | Полировка: документация и чек-лист                   | high      | S     | Done   | все задачи Этапов 1–4        |
...
- **2026-04-29** — закрыта задача 3.2: добавлен tool `read_document` в `app/tools/read_document.py` для чтения PDF/TXT/MD из временной директории. PDF через `pypdf`, TXT/MD через `Path.read_text`. Защита от path traversal, усечение до `max_chars`. Добавлен параметр конфигурации `TMP_FILES_DIR` (default `tmp`), зависимость `pypdf>=4.0`. Покрыто тестами: PDF, TXT, MD, усечение, защита от path traversal, неизвестные типы файлов.
- **2026-04-29** — закрыта задача 3.3: добавлен handler `handle_document` в `app/adapters/telegram/handlers/messages.py` для обработки документов из Telegram. Скачивает файл через `download_telegram_file`, формирует обогащённый goal с путём к файлу и caption, передаёт в `core.handle_user_task`. Обработка `FileTooLargeError` с понятным сообщением пользователю. Файл удаляется после обработки. Зарегистрирован в `messages_router` через фильтр `lambda m: m.document is not None`.
- **2026-04-29** — закрыта задача 3.4: добавлен сервис `Transcriber` в `app/services/transcribe.py` — обёртка над `faster-whisper` для транскрипции речи. Добавлен handler `handle_voice` в `messages.py` для обработки голосовых сообщений. Скачивает файл, транскрибирует через `Transcriber`, передаёт распознанный текст в `core.handle_user_task`. Добавлены параметры конфигурации `WHISPER_MODEL` (default `base`) и `WHISPER_LANGUAGE` (default `ru`). Добавлена зависимость `faster-whisper>=1.0` в requirements.txt. Если faster-whisper не установлен, handler отвечает понятным сообщением. Зарегистрирован в `messages_router` через фильтр `lambda m: m.voice is not None`.
- **2026-04-29** — закрыта задача 3.5: добавлен сервис `Vision` в `app/services/vision.py` — обёртка над `OllamaClient.chat` с поддержкой параметра `images` для описания изображений. Добавлен handler `handle_photo` в `messages.py` для обработки фотографий. Скачивает файл, описывает через `Vision`, передаёт описание в `core.handle_user_task`. Добавлен параметр конфигурации `VISION_MODEL` (default `None`). Если vision-модель не задана, handler отвечает fallback-сообщением. Caption пробрасывается как часть prompt. Зарегистрирован в `messages_router` через фильтр `lambda m: m.photo is not None`.
- **2026-04-29** — закрыта задача 3.6: обновлена документация по файловым входам. В `architecture.md` добавлен раздел 6 «Поток обработки файлов» с описанием handler'ов Document, Voice, Photo. В `commands.md` добавлен раздел «Файлы» с подробным описанием алгоритмов. В `requirements.md` добавлен раздел 1.7 «Медиа-файлы» с FR-34..FR-39. В `current-state.md` обновлен раздел «Что работает» с информацией о файловых входах и отмечено исправление проблемы потери ранней истории. В `roadmap.md` Этап 7 помечен как частично закрытый Спринтом 02. В `README.md` обновлен раздел «Возможности» и отмечено закрытие дыр MVP. В `_board/progress.txt` заполнен чек-лист приёмки Спринта 02 (все 9 пунктов выполнены).
- **2026-04-29** — исправлена поддержка vision: сделан `Vision.describe()` асинхронным, изображения передаются внутри сообщений вместо параметра `images`, добавлен `mime_type` в `download_telegram_file` для корректного определения расширений, создается `tmp_dir` при скачивании файлов. Обновлена документация во всех файлах с упоминанием `llava:7b` как vision-модели. Спринт 02 закрыт.
