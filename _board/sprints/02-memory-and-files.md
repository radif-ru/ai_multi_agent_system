...
- [ ] `_docs/memory.md` — новая секция «§2.5 Полный лог сессии» с описанием буфера и инвариантов.
- [ ] `_docs/memory.md` §3.3 — обновлено: `Archiver` принимает полный лог, а не in-session историю.
- [ ] `_docs/architecture.md` §3.5 — упомянут новый буфер `session_log`.
- [ ] `_docs/current-state.md` §2 — запись о ранее существовавшем баге (потеря ранней истории при `/new`).
- [ ] Тесты: n/a (документация).
...
- [ ] `add_user_message` / `add_assistant_message` дублируют запись в `_session_log`.
- [ ] `replace_with_summary` не модифицирует `_session_log` (тест).
- [ ] `get_session_log(user_id)` возвращает независимую копию (тест на изоляцию мутаций).
- [ ] `clear(user_id)` и `rotate_conversation_id(user_id)` обнуляют `_session_log` (тест).
- [ ] Переполнение `SESSION_LOG_MAX_MESSAGES` → отбрасывание головы + одно `WARNING` (тест).
- [ ] `Settings` — поле и валидатор; `.env.example` обновлён.
- [ ] `pytest -q` зелёный.
...
- [ ] `cmd_new` использует полный лог.
- [ ] Тест: после длинной сессии (≥ `history_summary_threshold` сообщений, in-session compaction сработал) `Archiver.archive` получает **полный** список без усечения.
- [ ] Тест: пустой лог → старое поведение (без вызова `Archiver`).
- [ ] Документ `_docs/commands.md` — секция `/new` уточнена (архивируется полный лог сессии).
- [ ] `pytest -q` зелёный.
...
- [ ] `Summarizer.summarize` поддерживает map-reduce; пороговое значение конфигурируется через `Settings.summarizer_chunk_messages`.
- [ ] `_prompts/summarizer.md` обновлён, явно требует сохранять факты о пользователе.
- [ ] Тесты: короткий лог → один вызов `llm.chat`; длинный лог → `ceil(N / chunk) + 1` вызовов; падение одного map-батча → `LLMError` пробрасывается, `Archiver` откатывает уже вставленные чанки.
...
| 4.1 | Диагноз и спецификация полного лога сессии           | high      | S     | Progress | —                          |
| 4.2 | `ConversationStore`: полный лог сессии               | high      | S     | ToDo   | 4.1                          |
| 4.3 | `cmd_new`: архивировать полный лог сессии            | high      | XS    | ToDo   | 4.2                          |
| 4.4 | Map-reduce суммаризация длинного лога                | medium    | M     | ToDo   | 4.3                          |
| 4.5 | Регрессионный e2e: имя переживает `/new`             | high      | S     | ToDo   | 4.3, 2.2                     |
