...
- [ ] Логирование показывает время каждого этапа `/new` (суммаризация, чанкинг, embedding, запись).
- [ ] Оптимизация: embedding чанков параллельный (asyncio.gather с семафором).
- [ ] Команда `/new` показывает прогресс пользователю («Суммирую…», «Создаю эмбеддинги…») при длительности > 5 секунд.
- [ ] Тест: `/new` на сессии из 50+ сообщений завершается < 30 секунд.
- [ ] **Документация обновлена:** `_docs/architecture.md` §5, `_docs/commands.md` §`/new`, `_docs/current-state.md` §2.
- [ ] **Тесты добавлены:** тест параллельного embedding, тест прогресса.
...
|-----|--------|:---------:|:-----:|:------:|:----------:|
| 1.1 | Диагностика и устранение таймаутов | high | M | Progress | — |
