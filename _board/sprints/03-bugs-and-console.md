...
- [x] `/new` работает быстро или даёт понятный прогресс, нет таймаутов.
- [x] Бот принимает новые скиллы (bash-linux, bash-pro, weather) без ошибок.
- [x] Консольный режим запускается и поддерживает все команды Telegram.
- [x] Команда для выбора поисковика работает в Telegram и консоли.
- [x] Reply-сообщения в Telegram обрабатываются с контекстом.
- [x] Код в ответах бота отформатирован для Telegram.
...
- Запуск: `python -m app.console_main` (отдельная entry point, не `__main__.py`).
...
- [x] Документ `_docs/console-adapter.md` со спецификацией.
- [x] Обновить `_docs/architecture.md` §8.4 — упомянуть консольный адаптер как пример.
- [x] Тесты: n/a (документация).
- [x] `git status` чист, `pytest -q` зелёный.
...
- [x] Модуль `app/commands/` с `CommandContext`, `CommandResult`, `CommandRegistry`.
- [x] Все команды (`/start`, `/help`, `/models`, `/model`, `/prompt`, `/new`, `/reset`) вынесены в `app/commands/`.
- [x] Telegram handler рефакторен: использует `app/commands/`.
- [x] Тесты для команд работают (обновить `tests/adapters/telegram/test_commands.py`).
- [x] **Документация обновлена:** `_docs/commands.md` (новая структура), `_docs/architecture.md`.
- [x] **Тесты добавлены/обновлены:** `tests/commands/test_*.py`.
- [x] `git status` чист, `pytest -q` зелёный.
...
- `app/console_main.py` — точка входа `python -m app.console_main`.
...
- [x] `python -m app.console_main` запускается и работает REPL.
- [x] Все команды `/start`, `/help`, `/models`, `/model`, `/prompt`, `/new`, `/reset` работают.
- [x] Текстовые сообщения обрабатываются через `core.handle_user_task`.
- [x] История между сообщениями сохраняется (in-memory).
- [x] **Документация обновлена:** `_docs/console-adapter.md`, `_docs/README.md` (навигация).
- [x] **Тесты добавлены:** `tests/adapters/console/test_adapter.py` (моки stdin/stdout).
- [x] `git status` чист, `pytest -q` зелёный.
...
- [x] Команды `/search_engines`, `/search_engine <name>` в `app/commands/`.
- [x] `UserSettingsRegistry` поддерживает `search_engine` per user.
- [x] Tool `web_search` использует активный поисковик (fallback на `duckduckgo`).
- [x] **Документация обновлена:** `_docs/commands.md` §«Выбор поисковика», `_docs/stack.md` §9.
- [x] **Тесты добавлены:** команды, tool с разными поисковиками.
- [x] `git status` чист, `pytest -q` зелёный.
...
- [x] Handler `handle_text` проверяет `message.reply_to_message`.
- [x] Если есть reply — добавляет контекст оригинального сообщения.
- [x] **Документация обновлена:** `_docs/architecture.md` §4, `_docs/commands.md`.
- [x] **Тесты добавлены:** reply с текстом, reply с длинным текстом (обрезка).
- [x] `git status` чист, `pytest -q` зелёный.
...
- [x] Добавить настройку `TMP_BASE_DIR` в `.env.example` (вместо `TMP_FILES_DIR`)
- [x] Изменить `config.py`: `tmp_files_dir` → `tmp_base_dir`, добавить метод `get_user_tmp_dir(user_id)`
- [x] Переименовать `image_contexts.db` → `files_contexts.db` в `ConversationStore`
- [x] Изменить схему БД: таблица `image_contexts` → `file_contexts`, добавить поле `file_type`
- [x] Обновить `download_telegram_file` для создания подкаталога по `user_id`
- [x] Обновить все вызовы `download_telegram_file` для передачи `user_id`
- [x] Сохранять контекст всех файлов (картинки, документы, голос) в БД
- [x] Обновить `cmd_new` для удаления каталога пользователя целиком
- [x] Обновить инструменты (`ReadDocumentTool`, `DescribeImageTool`) для использования `tmp_base_dir`
- [x] Обновить тесты для использования новой структуры
- [x] Обновить документацию (`_docs/memory.md`, `_docs/tools.md`)
- [x] Тесты: `pytest -q` зелёный.
- [x] `git status` чист.

---

### Задача 4.9. Сохранение файлов в рамках сессии

- **Статус:** Done
- **Приоритет:** high
- **Объём:** M
- **Зависит от:** 4.8
- **Связанные документы:** `_docs/memory.md` §2 (краткосрочная память).
- **Затрагиваемые файлы:** `app/adapters/telegram/handlers/messages.py`, `tests/adapters/telegram/test_documents.py`.

#### Описание

Текущая проблема:
1. Документы удаляются сразу после обработки, а не живут до `/new` как изображения
2. Голосовые файлы удаляются сразу после транскрипции
3. При reply на голосовой файл контекст не добавляется

Цели:
1. Убрать удаление файлов документов после обработки (файлы живут до `/new`)
2. Убрать удаление голосовых файлов после транскрипции
3. Добавить сохранение контекста голосовых файлов в БД с транскрипцией
4. Добавить обработку reply на голосовые файлы в handle_text

#### Definition of Done

- [x] Убрать удаление файла документа в `handle_document` после обработки
- [x] Обновить тест `test_handle_document_success` - файл не должен удаляться
- [x] Убрать удаление голосового файла в `handle_voice` после транскрипции
- [x] Добавить сохранение контекста голосового файла в БД через `save_file_context`
- [x] Добавить проверку на `voice` в `handle_text` для обработки reply на голосовые файлы
- [x] Тесты: `pytest -q` зелёный.
- [x] `git status` чист.

---

### Задача 4.10. Инструмент weather для получения погоды

- **Статус:** Done
- **Приоритет:** medium
- **Объём:** S
- **Зависит от:** —
- **Связанные документы:** `_skills/weather/SKILL.md`.
- **Затрагиваемые файлы:** `app/tools/weather.py`, `app/main.py`, `app/console_main.py`, `tests/tools/test_weather.py`.

#### Описание

Текущая проблема:
1. LLM пытается вызвать инструмент `weather`, но он не существует
2. Weather это skill в `_skills/weather/SKILL.md`, но не реализован как инструмент

Цели:
1. Создать инструмент `WeatherTool` для получения погоды через wttr.in
2. Использовать curl для запроса к wttr.in
3. Добавить автоматический fallback на WebSearchTool если wttr.in недоступен
4. Зарегистрировать инструмент в ToolRegistry
5. Добавить тесты для WeatherTool

#### Definition of Done

- [x] Создать `app/tools/weather.py` с WeatherTool
- [x] Использовать curl для запроса к wttr.in
- [x] Добавить fallback на WebSearchTool при ошибках сети
- [x] Зарегистрировать WeatherTool в ToolRegistry в main.py
- [x] Зарегистрировать WeatherTool в ToolRegistry в console_main.py
- [x] Создать тесты для WeatherTool в `tests/tools/test_weather.py`
- [x] Тесты: `pytest -q` зелёный.
- [x] `git status` чист.

---

### Задача 4.11. Исправление формата final_answer в системном промпте

- **Статус:** Done
- **Приоритет:** high
- **Объём:** XS
- **Зависит от:** —
- **Связанные документы:** `_docs/agent-loop.md` §2 (формат JSON-ответа модели).
- **Затрагиваемые файлы:** `_prompts/agent_system.md`, `app/agents/protocol.py`.

#### Описание

Текущая проблема:
1. LLM пытается использовать `final_answer` как инструмент (`action: "final_answer"`)
2. `final_answer` это НЕ инструмент, а специальное поле для завершения цикла
3. Правильный формат: `{"final_answer": "ответ"}`, а не `{"action": "final_answer", "args": {}}`

Цели:
1. Усилить описание формата `final_answer` в системном промпте
2. Добавить явное предупреждение что `final_answer` НЕ инструмент
3. Предотвратить использование `final_answer` как значения поля `action`

#### Definition of Done

- [x] Добавить явное предупреждение в `_prompts/agent_system.md` что `final_answer` НЕ инструмент
- [x] Добавить предупреждение "Никогда не используй final_answer как значение поля action"
- [x] Тесты: `pytest -q` зелёный.
- [x] `git status` чист.
...
| 2.1 | Спецификация консольного режима | high | S | Done | — |
...
| 3.3 | Обработка reply | medium | S | Done | — |
...
| 4.8 | Изоляция файлов по пользователям | high | M | Done | — |
...
- **2026-05-04** — задача 4.8 Done: изоляция файлов по пользователям — отдельные каталоги data/tmp/{user_id}/, переименование image_contexts.db в files_contexts.db, сохранение контекста всех файлов (картинки, документы), очистка каталога пользователя при /new.
- **2026-05-04** — задача 4.9 Done: сохранение файлов в рамках сессии — документы и голосовые файлы не удаляются после обработки (живут до `/new`), добавлено сохранение контекста файлов через `save_file_context` в ConversationStore, добавлена обработка reply на файлы в `handle_text` с использованием `get_file_context`.
...
- **2026-05-04** — задача 4.12 Done: добавлена поддержка OCR для PDF — `pytesseract`+`Pillow` в `requirements.txt`, настройки `READ_DOCUMENT_OCR_ENABLED`/`READ_DOCUMENT_MAX_OCR_IMAGES`, кеш `.ocr.txt`, авто-выбор `rus+eng`/`eng` по доступным языкам tesseract, документация `_docs/tools.md`. Примечание: дисковый кеш `.ocr.txt` позже выведен из эксплуатации в задаче 06.3-bis.4 (результат OCR теперь живёт в `dialog_journal.content`).
- **2026-05-04** — задача 4.13 Done: при `LLMBadResponse` в Telegram-хендлере к ответу пользователю добавлены детали исключения; обновлён тест `test_llm_bad_response_replies_with_format_message`.
- **2026-05-04** — задача 4.14 Done: настраиваемые уровни логирования — добавлены `LOG_LEVEL_CONSOLE=DEBUG` и `LOG_LEVEL_FILE=INFO` в `.env.example`/`config.py`; `logging_config.py` использует их вместо хардкода.
- **2026-05-04** — спринт закрыт (см. `_board/plan.md` § «Закрытые»).
...
- **2026-05-04** — улучшение goal для handle_document: добавлено явное указание использовать ТОЛЬКО полный путь к файлу без изменений, чтобы агент не передавал относительные пути.
- **2026-05-04** — добавлено логирование аргументов в tool registry для отладки.
- **2026-05-04** — улучшена обработка пустых ответов LLM: при пустом ответе показывается более понятное сообщение пользователю "Модель не смогла обработать такой большой запрос. Попробуйте отправить файл меньшего размера или задайте более конкретный вопрос."
- **2026-05-04** — добавлена автоматическая суммаризация контекста: настройка AGENT_MAX_CONTEXT_CHARS (default 8000) в config.py и .env.example, executor автоматически суммаризирует историю если контекст превышает лимит перед отправкой в LLM, что предотвращает пустые ответы при больших контекстах (например, при обработке PDF с OCR текстом).
- **2026-05-04** — актуализирована документация: architecture.md добавлена обработка пустых ответов LLM и автоматическая суммаризация контекста, README.md добавлено упоминание про AGENT_MAX_CONTEXT_CHARS.
- **2026-05-04** — спринт закрыт, все задачи выполнены.
