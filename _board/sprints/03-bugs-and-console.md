...
- [x] Кросс-ссылка `§7.4` → `§8.4` исправлена в этом же файле спринта (Задача 2.1) и в `_docs/roadmap.md` (Этап 5).
...
- [ ] Закрытые спринты (`sprints/00-*`, `sprints/01-*`, `sprints/02-*`) — **не трогаем** (правило «история неизменна»).
- [ ] `git grep -n 'plan\.md §'` и `git grep -n 'process\.md §'` и `git grep -n 'instructions\.md §'` не дают битых ссылок (кроме исторических в закрытых спринтах).
- [ ] Тесты: n/a (документация).
- [ ] `pytest -q` зелёный, `git status` чист.
...
| 4.7 | Реорганизация документации `_board/` и `instructions.md` | medium | S | Progress | — |
