...
- [ ] Top-level wrapper `run()` в `app/main.py` и `app/console_main.py` логирует необработанные исключения через `logger.exception(...)` и пробрасывает их дальше; `KeyboardInterrupt` пробрасывается без лога.
...
- [ ] `app/main.py::run` и `app/console_main.py::run` обновлены по описанию.
- [ ] `tests/test_main.py` содержит хотя бы один тест на каждую точку входа: `RuntimeError` пробрасывается, `caplog` содержит запись `ERROR` с traceback; отдельный кейс: `KeyboardInterrupt` пробрасывается без записи `ERROR`.
- [ ] `pytest -q` и `flake8 app tests` зелёные.
- [ ] **Документация обновлена** — `_docs/current-state.md` §3 (нюанс про top-level логирование).
- [ ] **Тесты добавлены / обновлены** — да.
- [ ] `git status` чист.
...
| 6.1 | Логировать необработанные исключения в `run()`        | medium    | XS    | Progress | —               |
