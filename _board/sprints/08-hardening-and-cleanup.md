...
5. Обновить документацию с описанием решения.

#### Definition of Done

- [ ] Monkey-patch aiohttp добавлен, proxy параметр передан в Bot (бот подключается к Telegram API через прокси).
- [ ] Сервис запускается и отвечает на сообщения без timeout ошибок.
- [ ] `_docs/current-state.md` обновлён с описанием решения.
- [ ] **Документация обновлена** — да.
- [ ] **Тесты добавлены / обновлены** — `n/a` (инфраструктурное изменение).
- [ ] `git status` чист.
...
| 8.1 | Настроить aiogram на использование системного прокси | high      | S     | Progress | —                 |
...
- **2026-05-21** — закрыта задача 08.2.1: SQLite-`UserRepository` с таблицей `users` в `data/memory.db`; `init/close` в точках входа; `UserCreated` публикуется ровно при `INSERT`; регрессия на рестарт в `tests/users/test_repository.py`; документация `_docs/current-state.md` §1.5/§3/§6.4 обновлена.
- **2026-05-21** — закрыта задача 08.3.1: удалены `app/services/file_contexts_migration.py`, `tests/services/test_file_contexts_migration.py` и вызовы в `app/main.py`/`app/console_main.py`; `_docs/memory.md` §2.6.1/§4.1, `_docs/security.md` §2, `_docs/current-state.md` §1.7/§6.5 обновлены.
- **2026-05-21** — закрыта задача 08.4.1: регрессионный `tests/services/test_archiver.py::test_archive_completes_within_budget` с бюджетом < 2.0s и маркером `slow`; маркер зарегистрирован в `pyproject.toml`, описан в `_docs/testing.md` §2.
- **2026-05-21** — закрыта задача 08.5.1: финальные записи в `_docs/current-state.md` (§1.5, §3, §6.4, §6.5); в `roadmap.md` правок не требуется.
