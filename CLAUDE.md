...
Эти правила применяются всегда, без исключений, и не являются «компромиссом» — они приоритетнее любых соображений скорости и краткости:

- **Язык — общее правило проекта.** По умолчанию русский, но не строго: если на другом языке смысл понятнее — используется он. Касается всего: чат, рассуждений, коммитов, комментариев, докстрингов, документов, **логов**, сообщений бота. Полное правило и список исключений — в `_docs/instructions.md` §0.
...
- **Обязательное тестирование при изменении кода.** Любая задача, затрагивающая код в `app/`, должна быть покрыта тестами на новое поведение. Перед коммитом кодовых изменений прогоняется `pytest -q` — коммит только при зелёном результате. См. `_docs/instructions.md` §7.1–§7.2.
- **Обязательное обновление документации.** Перед коммитом кодовых изменений, меняющих поведение системы, должна быть обновлена соответствующая документация в `_docs/`. Коммит кода без обновления документации не допускается. См. `_board/process.md` §10.

Перед началом любой работы по задаче — перечитать §4, `_docs/instructions.md` §0 и `_board/process.md`.
