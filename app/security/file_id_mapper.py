...
def file_id_not_found_message(file_id: str) -> str:
    ...
    return (
        ...
        "полным содержимым файла, его нужно загрузить заново. И предупреди, "
        "что следующая команда /new опять удалит загруженные файлы, поэтому "
        "использовать её стоит с осторожностью."
    )
