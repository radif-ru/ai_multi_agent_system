...
**Статус:** Active (Консольный адаптер закрыт Спринтом 03, 2026-05-04; web/MAX — Planned).

Архитектурно адаптеры подключаются единым контрактом `core.handle_user_task(text, user_id, chat_id)` (см. `architecture.md` §8.4).

- [x] **Консольный адаптер**: REPL-цикл с командами через общий модуль `app/commands/` (Спринт 03, задачи 2.1–2.3).
...
## Этап 6.5. События и пользователи

**Статус:** Closed (Спринт 04, 2026-05-05).

Введение событийной шины EventBus для развязки компонентов и модуля Users для управления пользователями.

- [x] **Модуль Users**: модель User и UserRepository с get_or_create (Спринт 04, задача 1.1).
- [x] **Интеграция UserRepository**: прокид в DI, использование в хендлерах (Спринт 04, задача 1.2).
- [x] **EventBus**: реализация pub/sub шины с гарантией порядка подписчиков (Спринт 04, задача 2.1).
- [x] **События**: UserCreated, MessageReceived, ResponseGenerated, ConversationArchived (Спринт 04, задача 2.2).
- [x] **Перенос записи в ConversationStore на подписчики**: conversation_subscriber (Спринт 04, задача 2.3).
- [x] **Перенос in-session суммаризации на подписчик**: summarizer_subscriber (Спринт 04, задача 2.4).
- [x] **Публикация ConversationArchived в Archiver**: событие при успешном архивировании (Спринт 04, задача 3.1).
- [x] **Очистка tmp-изображений на событие**: tmp_cleanup подписчик на ConversationArchived (Спринт 04, задача 3.2).

**Когда:** завершено Спринтом 04.
...
**Статус:** Closed (закрыт Спринтом 02 «Память и файловые входы», 2026-04-29; OCR PDF закрыт Спринтом 03, 2026-05-04; видео, location — backlog).
...
- [x] OCR PDF с поддержкой кириллицы через tesseract-ocr-rus (Спринт 03, задача 4.12).
- [ ] Видео, location — отдельный спринт по запросу.

## Этап 8.5. Усиление безопасности и OCR-рефакторинг

**Статус:** Closed (Спринт 05, 2026-05-06).

Внедрение мер защиты от типичных атак на LLM-системы и рефакторинг OCR-логики.

- [x] **Модуль безопасности**: InputSanitizer для защиты от prompt injection (Спринт 05, задача 3.1).
- [x] **Интеграция InputSanitizer**: в Telegram-хендлеры и консольный адаптер (Спринт 05, задача 3.2).
- [x] **Маскирование путей**: FileIdMapper для data leakage защиты (Спринт 05, задачи 4.1–4.2).
- [x] **Усиление system prompt**: добавление правил безопасности в агентный промпт (Спринт 05, задача 5.1).
- [x] **Защита tools**: allowlist для опасных tools и валидация параметров (Спринт 05, задачи 6.1–6.2).
- [x] **Фильтрация ответов**: ResponseSanitizer для маскирования системной информации (Спринт 05, задачи 7.1–7.2).
- [x] **OCR-сервис**: выделение OCR-логики в app/services/ocr.py (Спринт 05, задача 1.1).
- [x] **Tool ocr_image**: tool для распознавания текста с одиночных изображений (Спринт 05, задача 2.1).
- [x] **Skill ocr**: правила выбора между OCR и Vision (Спринт 05, задача 2.2).

**Когда:** завершено Спринтом 05.
