...
Документ описывает правила, которым должен следовать разработчик (и/или LLM-ассистент) при работе над проектом. Общие поведенческие гайдлайны LLM-агента (think before coding, simplicity, surgical changes, goal-driven) — `CLAUDE.md` в корне. Процесс работы со спринтами и задачами (открытие, выполнение, закрытие) — `_board/process.md`.
...
- Текст лог-сообщений — по §1. Структурные поля и идентификаторы (`tool=<name>`, `dur_ms`, `status`, имена моделей, URL'ы) — латиницей.
...
- **Только относительные пути.** В любой документации (`_docs/`, `_board/`, `README.md`, `.env.example`, `_skills/`, `_prompts/`, `CLAUDE.md`) запрещены абсолютные пути файловой системы (`/home/...`, `/Users/...`, `C:\...`). Все ссылки — относительно корня проекта (`_docs/...`, `_board/...`, `app/...`, `./architecture.md`).
- **Синхронизация ссылок.** При изменении названия/номера раздела, переименовании/удалении файла или переносе пункта — пройти `grep -rn` по всему репо и поправить все живые ссылки. Закрытые спринты (`_board/sprints/<NN>-*.md` со статусом `Closed`) — **не трогаем**, они архив (см. `_board/process.md` §2 п.5).
