...
- Коммиты — атомарные, в императиве, по [Conventional Commits](https://www.conventionalcommits.org/). Язык сообщения — **по умолчанию русский**, но не строго: если на другом языке смысл понятнее (имена символов, термины из кода, англоязычные технические выражения, служебные коммиты вроде `chore(plan): start task ...`) — используется он. Смешивать языки в одном сообщении допустимо.
