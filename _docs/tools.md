...
- **Большие ответы обрезаются** до `MAX_TOOL_OUTPUT_CHARS` (значение фиксируется в `app/config.py`, default — 50000 символов). Усечение — суффиксом `... [truncated]`.
...
- **Описание:** Распознать текст с изображения через OCR. Используется для точной транскрипции текста (сканы документов, чеки, таблицы).
- **Args:** `{"image_path": "<строка>", "lang": "<строка, optional>"}`.
- **Return:** распознанный текст с изображения; если текст не найден — сообщение "OCR не нашёл текста на изображении".
- **Реализация:** валидация пути (должен быть в `tmp/`, без `..`, расширение .jpg/.jpeg/.png/.gif/.webp); OCR делегируется сервису `app/services/ocr.py`; кеш OCR в `<file>.ocr.txt`; обрезка до 8000 символов.
