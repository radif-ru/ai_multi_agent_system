...
### 2.2 Парсер ответа агента не снимает markdown-fence (`` ```json ... ``` ``)

**Файл:** `app/agents/protocol.py:42` (`parse_agent_response`). **Серьёзность:** высокая.

`Executor` ожидает от модели ровно один JSON-объект, а `parse_agent_response` делает `json.loads(text)` без предобработки. На практике `qwen3.5:4b` в значительной доле ответов оборачивает JSON в markdown-fence — типично:
...
`json.loads` такой текст распарсить не может, бросает `LLMBadResponse`, `Executor` пишет `WARNING parse_error raw='```json\n...```'` и Telegram-handler отвечает пользователю «Модель ответила в неожиданном формате, попробуйте ещё раз». Воспроизведено в `logs/agent.log` (3 случая за одну сессию 2026-04-29: `step=1 kind=parse_error raw='\`\`\`json...\`\`\`'`).

**Рекомендация:** в `parse_agent_response` перед `json.loads` снимать обрамляющие markdown-fence (`\`\`\`json ... \`\`\`` или `\`\`\` ... \`\`\``), а также пробельные обрамления. Дополнительно — в системном промпте ужесточить требование «ответ — голый JSON, без markdown-кода». Покрыто задачей спринта 02 (Этап 5).
